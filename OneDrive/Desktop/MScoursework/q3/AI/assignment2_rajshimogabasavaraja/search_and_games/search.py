...
def iterativeDeepeningSearch(problem):
    ...
    while True: # repeat search with the depth increases until we find the goal
        ...
        while not problem.goalTest(state): #while we do not find the goal point
            successors = []
            validActions=[]
            validActions=problem.getActions(state)
            for action in validActions:
                nextState = problem.getResult(state, action)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not priority_queue.isEmpty():
        ...
        if top[0] not in visited_state:
            ...
            successors = []
            validActions=[]
            validActions=problem.getActions(top[0])
            for action in validActions:
                nextState = problem.getResult(top[0], action)
                cost=problem.getCost(top[0], action)
                successors.append( ( nextState, action, cost) )        
                    
            for succ, act, co in successors:
